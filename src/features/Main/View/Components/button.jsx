...
export default function Button({text, onClick, icon, withIcon = false, isPrimaryColor = false, isTransparent = false}) {
  ...
  return (
    <div onClick={()=>onClick()} className={`${isTransparent ? "bg-transparent" : isPrimaryColor ? primaryColor : normalColor} ${isPrimaryColor ? "text-text-dark" : ""}  cursor-pointer  ${icon ? "rounded-full p-2 " : "rounded px-2 md:px-4 py-2"}  transition duration-200`}>
